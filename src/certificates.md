...
The self-signed certificates are organised into a root CA for many service certificats to be authorised via.

The root CA is installed in any trusting hoost, including the development host for the docker network.

The trusted host recieves its host identification certificate as required.

With the use of a reverse proxy only the proxy itself requries a service certificate for standard web service.

However, as we are using an OAuth/ OpenID identity service that servies this outside and inside of the docker network, it also requries its exposure inside the docker network via SSL to be trusted by each of the trusting API service.


The certificate generation, fconfiguration and consumption is split into the following cases.
